...
This URL part is optional for codeforces problems. However, it will be used to construct file name if provided.
if provided.
.PARAMETER Occasion
...
param(
  ...
  [string]$Occasion,
  [string]$Path,
  [string]$OutDirPath = $Env:PS_SC_DRIVE + '\git_ws\Problem-Solving',
  [bool]  $Force = $false)
...
function Main() {
  switch ( $Command ) {
    ...
    'tempLintCmd' {
      ...
      $defaultStr = $Env:PS_SC_DRIVE + '\git_ws\Problem-Solving'
